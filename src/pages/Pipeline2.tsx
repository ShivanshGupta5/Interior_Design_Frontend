...
export default function Pipeline2() {
  ...
  const [room, setRoom] = useState<File | null>(null);
  const [roomPreview, setRoomPreview] = useState<string | null>(null);
  const [items, setItems] = useState<Item[]>([{ name: "", file: null, preview: null }]);
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState<string | null>(null);
  const [statusText, setStatusText] = useState<string | null>(null);
  const [jobId, setJobId] = useState<string | null>(null);
  ...
  const addItem = () => {
    if (items.length < 7) setItems([...items, { name: "", file: null, preview: null }]);
  };
  ...
  const updateItem = (idx: number, patch: Partial<Item>) => {
    ...
    if (patch.file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        next[idx] = { ...updated, preview: reader.result as string };
        setItems([...next]);
      };
      reader.readAsDataURL(patch.file);
      return;
